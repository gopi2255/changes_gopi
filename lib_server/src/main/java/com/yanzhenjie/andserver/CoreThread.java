...
class CoreThread extends Thread {
    ...
    private HttpParams createHttpParams() {
        return new BasicHttpParams()
                ...
                .setIntParameter(CoreConnectionPNames.SOCKET_BUFFER_SIZE, 1024 * 1024)
        ...
        if (mWebSite !=null) {
            ...
            if (websiteMap.size() > 0) {
                for (Map.Entry<String, RequestHandler> handler : websiteMap.entrySet()) {
                    handlerRegistry.register(BasicWebsite.getHttpPath(handler.getKey()), new DefaultHttpRequestHandler(handler.getValue()));
                }
            }
        }
