...
import static com.awesomeplayer.R.id.*;
...
public class GiraffePlayer {
    ...
    private final View.OnClickListener onClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (v.getId() == app_video_fullscreen) {
                toggleFullScreen();
    ...
    private long defaultRetryTime = 1000;
    Handler handlerPostion = new Handler();
    private Runnable runnablePosition = new Runnable() {
        @Override
        public void run() {
            if (onPositionListener != null && videoView != null && !isDragging && !error) {
                long position = videoView.getCurrentPosition();
                onPositionListener.onChanged(position);
            }
            if (runnablePosition != null)
                handlerPostion.postDelayed(runnablePosition, 3000);
        }
    };
    ...
    private OnPositionListener onPositionListener = new OnPositionListener() {
        @Override
...
//    }
    ...
    public void show(int timeout) {
        if (!isShowing) {
//            $.id(R.id.layoutCustomActionBar).visible();
            if (!isLive) {
                showBottomControl(true);
            }
            if (!fullScreenOnly) {
                $.id(app_video_fullscreen).visible();
            }
            isShowing = true;
    ...
    private void showBottomControl(boolean show) {
        $.id(R.id.customBottomBar).visibility(show ? View.VISIBLE : View.GONE);
    }
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            ...
            if (instantSeeking) {
                ...
                if (onPositionListener != null) {
                    onPositionListener.onChanged(newPosition);
                }
            }
...
//                        Toaster.show("download rate:" + extra);
        ...
        $.id(app_video_fullscreen).clicked(onClickListener);
...
//        $.id(R.id.layoutCustomActionBar).gone();

        $.id(app_video_fullscreen).gone();
...
//            $.id(layoutCustomActionBar).gone();
    ...
    private void updateFullScreenButton() {
        if (getScreenOrientation() == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
            $.id(app_video_fullscreen).image(R.drawable.ic_fullscreen_exit_white_36dp);
        } else {
            $.id(app_video_fullscreen).image(R.drawable.ic_fullscreen_white_24dp);
        }
    }

    public void setFullScreenOnly(boolean fullScreenOnly) {
        if (this.fullScreenOnly) return;
...
//            activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
        ...
        screenWidthPixels = activity.getResources().getDisplayMetrics().heightPixels;
...
//        $.id(R.id.app_video_finish).visibility(show ? View.VISIBLE : View.GONE);
    ...
    public GiraffePlayer setOnPositionListener(OnPositionListener onPositionListener) {
