package com.umpee.camerafragment.internal.manager.impl;
...
import com.umpee.camerafragment.internal.ui.model.PhotoQualityOption;
import com.umpee.camerafragment.internal.ui.model.VideoQualityOption;
import com.umpee.camerafragment.internal.utils.CameraHelper;
...
public class Camera1Manager extends BaseCameraManager<Integer, SurfaceHolder.Callback> {
    ...
    public void startVideoRecord(final File videoFile, CameraVideoListener cameraVideoListener) {
        ...
        if (videoListener != null) backgroundHandler.post(new Runnable() {
            @Override
            public void run() {
    ...
    public void stopVideoRecord(@Nullable final CameraFragmentResultListener callback) {
        if (isVideoRecording) backgroundHandler.post(new Runnable() {
            @Override
            public void run() {
    ...
    protected void prepareCameraOutputs() {
        try {
            ...
            videoSize = CameraHelper.getSizeWithClosestRatio((videoSizes == null || videoSizes.isEmpty()) ? previewSizes : videoSizes, camcorderProfile.videoFrameWidth, camcorderProfile.videoFrameHeight);

            photoSize = CameraHelper.getPictureSize((pictureSizes == null || pictureSizes.isEmpty()) ? previewSizes : pictureSizes, configurationProvider.getMediaQuality() == Configuration.MEDIA_QUALITY_AUTO ? Configuration.MEDIA_QUALITY_HIGHEST : configurationProvider.getMediaQuality());
...
////            videoRecorder.setVideoEncodingBitRate(800*1024);
...
//            // videoRecorder.setVideoSize(640, 480);
//            // videoRecorder.setVideoEncodingBitRate(camcorderProfile.videoBitRate);
//            videoRecorder.setVideoEncodingBitRate(1024*1024);
//            // videoRecorder.setVideoEncoder(camcorderProfile.videoCodec);
...
//            videoRecorder.setVideoSize(videoSize.getWidth(), videoSize.getHeight());
////            videoRecorder.setVideoSize(640, 480);
//            videoRecorder.setVideoEncodingBitRate(1000000);
...
//            videoRecorder.setAudioEncodingBitRate(camcorderProfile.audioBitRate);
//            videoRecorder.setAudioChannels(camcorderProfile.audioChannels);
//            videoRecorder.setAudioSamplingRate(MediaRecorder.VideoSource.DEFAULT);
//            videoRecorder.setAudioEncoder(MediaRecorder.AudioSource.DEFAULT);
//
    ...
    private void startPreview(SurfaceHolder surfaceHolder) {
        try {
            ...
            if (Build.VERSION.SDK_INT > 13 && (configurationProvider.getMediaAction() == Configuration.MEDIA_ACTION_VIDEO || configurationProvider.getMediaAction() == Configuration.MEDIA_ACTION_UNSPECIFIED)) {
//                parameters.setRecordingHint(true);
            }

            if (Build.VERSION.SDK_INT > 14 && parameters.isVideoStabilizationSupported() && (configurationProvider.getMediaAction() == Configuration.MEDIA_ACTION_VIDEO || configurationProvider.getMediaAction() == Configuration.MEDIA_ACTION_UNSPECIFIED)) {
